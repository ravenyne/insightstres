        ...
        <main class="flex-1 overflow-y-auto lg:ml-64">
            ...
            <header class="hidden lg:block bg-slate-800/30 border-b border-slate-700 px-8 py-6">
                <div>
                    ...
                    <p class="text-slate-400 mt-1">Kelola feedback dari pengguna</p>
                </div>
            </header>
            ...
            <div class="p-4 lg:p-8">
                ...
                <div class="bg-slate-800/50 border border-slate-700 rounded-2xl p-6 mb-6">
                    <form method="GET" action="{{ route('admin.feedback') }}" class="flex flex-wrap gap-4">
                        <div class="flex-1 min-w-[200px]">
                            ...
                            <select name="status" class="w-full px-4 py-3 bg-slate-900/50 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition">
                        ...
                        <div class="flex-1 min-w-[200px]">
                            ...
                            <select name="type" class="w-full px-4 py-3 bg-slate-900/50 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition">
                        ...
                        <div class="flex-1 min-w-[200px]">
                            ...
                            <select name="rating" class="w-full px-4 py-3 bg-slate-900/50 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition">
                        ...
                        <div class="flex items-end">
                            <button type="submit" class="px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-lg transition">
                                Filter
                            </button>
                        </div>
                    </form>
                ...
                <div class="bg-slate-800/50 border border-slate-700 rounded-2xl overflow-hidden">
                    <div class="px-6 py-4 border-b border-slate-700">
                        <h3 class="text-lg font-bold">Daftar Feedback ({{ $feedback->total() }})</h3>
                    </div>
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="border-b border-slate-700 bg-slate-800/30">
                                    <th class="text-left py-3 px-4 text-slate-400 font-medium text-sm">User</th>
                                    <th class="text-left py-3 px-4 text-slate-400 font-medium text-sm">Type</th>
                                    <th class="text-left py-3 px-4 text-slate-400 font-medium text-sm">Subject</th>
                                    <th class="text-left py-3 px-4 text-slate-400 font-medium text-sm">Rating</th>
                                    <th class="text-left py-3 px-4 text-slate-400 font-medium text-sm">Status</th>
                                    <th class="text-left py-3 px-4 text-slate-400 font-medium text-sm whitespace-nowrap">Date</th>
                                    <th class="text-left py-3 px-4 text-slate-400 font-medium text-sm">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($feedback as $item)
                                    <tr class="border-b border-slate-700/50 hover:bg-slate-700/30 transition">
                                        ...
                                        <td class="py-3 px-4">
                                            ...
                                            @endphp
                                            <span class="inline-block px-2 py-1 {{ $typeColor['bg'] }} {{ $typeColor['text'] }} text-xs font-semibold rounded-full whitespace-nowrap">
                                                {{ ucfirst($item->type) }}
                                            </span>
                                        </td>
                                        <td class="py-3 px-4">
                                        ...
                                        <td class="py-3 px-4">
                                            @if($item->rating)
                                                <div class="flex items-center gap-0.5">
                                                    @for($i = 1; $i <= 5; $i++)
                                                        <svg class="w-3.5 h-3.5 {{ $i <= $item->rating ? 'text-yellow-400 fill-yellow-400' : 'text-slate-600' }}" fill="currentColor" viewBox="0 0 20 20">
                                                ...
                                                <div x-show="open" x-transition @click.stop class="absolute right-0 mt-2 w-56 bg-slate-800 border border-slate-700 rounded-lg shadow-xl z-50">
                                                    <div class="py-1">
                                                        ...
                                                        <div class="px-4 py-2">
                                                            ...
                                                            <select onchange="updateStatus({{ $item->id }}, this.value)" class="w-full px-3 py-1.5 bg-slate-900/50 border border-slate-600 rounded text-sm text-white focus:outline-none focus:border-orange-500">
            ...
            <div id="feedbackModal" class="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
                <div class="bg-slate-800 rounded-2xl border border-slate-700 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="sticky top-0 bg-slate-800 border-b border-slate-700 px-6 py-4 flex items-center justify-between">
                        <h3 class="text-xl font-bold text-white">Feedback Details</h3>
                ...
                if (data.success) {
                    ...
                    content.innerHTML = `
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <p class="text-sm text-slate-400 mb-1">Type</p>
                                <span class="inline-block px-3 py-1 ${typeColor.bg} ${typeColor.text} text-sm font-semibold rounded-full">
                            ...
                            <div>
                                <p class="text-sm text-slate-400 mb-1">Rating</p>
                                <div class="flex items-center gap-1">
                                    ...
                                    ).join('') : '<span class="text-slate-500 text-sm">No rating</span>'}
                                </div>
                            </div>
                            <div>
                        ...
                        <!-- Subject -->
                        <div>
                            <p class="text-sm text-slate-400 mb-2">Subject</p>
                            <p class="text-white font-medium text-lg">${feedback.subject}</p>
                        </div>

                        <!-- Message -->
                        <div>
                            <p class="text-sm text-slate-400 mb-2">Message</p>
                            <div class="bg-slate-900/50 rounded-xl p-4 border border-slate-700">
                        ...
                        ${feedback.page_url ? `
                        <div>
                            <p class="text-sm text-slate-400 mb-2">Page URL</p>
                            <a href="${feedback.page_url}" target="_blank" class="text-orange-400 hover:text-orange-300 text-sm break-all">
                                ${feedback.page_url}
                            </a>
                        </div>
                        ` : ''}
                        ...
                        <div>
                            ...
                            <textarea id="adminNotes" rows="4" class="w-full px-4 py-3 bg-slate-900/50 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition" placeholder="Add internal notes...">${feedback.admin_notes || ''}</textarea>
                            <button onclick="saveAdminNotes(${feedback.id})" class="mt-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white font-medium rounded-lg transition">
                                Save Notes
                            </button>
                        </div>
                    `;
